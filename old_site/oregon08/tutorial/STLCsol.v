...
(** An interactive tutorial on developing programming language
    ...
    Distribution site: http://plclub.org/coq-tutorial/
*)
...
(** We use a locally nameless representation for the simply-typed
    ...
    The type [atom], defined in the Atom library, represents names.
    Equality on names is decidable ([eq_atom_dec]), and it is possible
    to generate an atom fresh for any given finite set of atoms
    ([atom_fresh_for_set], in the AtomSet library).
*)
...
(* Demo [atomsetdec]
   ...
   The tactic [atomsetdec] solves a certain class of propositions
   involving finite sets. See the documentation in [FSetWeakDecide]
   for a full specification.
*)

Lemma atomsetdec_demo : forall (x :atom) (S : atoms),
...
(** Exercise [subst_fresh]
    ...
    - Part of this proof includes a false assumption about free
      ...
      finite set membership that is solvable by [atomsetdec].
*)
...
Proof.
  ...
  Case "fvar".
    ...
    destruct (a == x).
    SCase "a=x".
      subst. simpl fv in H. atomsetdec.
    SCase "a<>x".
      auto.
  Case "abs".
    simpl.
...
(* Take-home Demo: Prove that free variables are not introduced by
   ...
   are the tactic [simpl_notin_hyps] and the lemma [notin_union], both
...
Proof.
  ...
  Case "bvar".
    ...
    simpl. auto.
  Case "fvar".
    destruct (a == x). subst. simpl in FV. atomsetdec. auto.
  Case "abs".
...
(** When picking a fresh atom or applying a rule that uses cofinite
    ...
    tactic, [ltac_map] (from the AdditionalTactics, which is imported
    as part of the Metatheory library), which maps a function over all
    hypotheses wit the appropriate type.  In this case, the argument
    to each invocation of [ltac_map] is a function that should return
    the set of atoms appearing in its argument.
*)
...
Ltac gather_atoms :=
  ...
  let D := ltac_map (fun x : exp => fv x) in
...
(*  For STLC, environments bind [atom]s to [typ]s.  We define an
    ...
    The Metatheory library extends this reasoning with the [AtomEnv]
    module that supports reasoning about association lists whose keys
    are [atom]s.  Everything in this library is polymorphic over the
    type of objects bound in the environment.  Look in [AtomEnv] for
    additional details about the functions and predicates that we
    mention below.
*)
...
(* To make environments easy to read, instead of building them from [nil]
   and [cons], we prefer to build them from the following
   components:
      - [nil]: The empty list.
...
(* Note that the [simpl] tactic doesn't produce the "normal form" for
   ...
   Furthermore, to convert an environment to any equivalent form other
   than the normal form (perhaps to apply a lemma) use the tactic
   [rewrite_env].
  *)

Lemma rewrite_env_demo : forall (x y:atom) (T:typ) P, (forall E, P ((x,T):: E) -> True) ->
  P (x ~ T) -> True.
Proof.
  intros x y T P H.
  (* apply H. fails here. *)
       ...
       - The [typing_abs_c] case follows from the induction
         ...
         things as you might expect.
           ...
           -- Here, use [auto] to solve facts about finite sets of
              ...
              the scenes.  [atomsetdec] does not work with the [dom]
              function.
...
(***********************************************************************)
(* Problemmatic proof of weakening. *)
(***********************************************************************)
