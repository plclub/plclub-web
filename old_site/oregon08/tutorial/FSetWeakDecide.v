(* Copyright (c) 2008 University of Pennsylvania PLClub
   ...
   $Id: FSetWeakDecide.v 2189 2008-07-15 12:57:07Z baydemir $

   Original author: Aaron Bohannon *)
...
*)
  ...
  Module FSetDecideAuxiliary.
    ...
    (** ** Turning Set Operators into Propositional Connectives
        The lemmas will be used to break down set operations
        into propositional formulas built over the predicates
        [In] and [E.eq] applied only to variables.  We are going
        to use them with [autorewrite].  (There were copied from
        the [Facts] functor to break that module dependency.)
        *)
    ...
    Section Iff.
      ...
      Proof.
        clear. intuition. apply (empty_1 H).
      Qed.
      ...
      Proof.
        split; [split; [apply remove_3 with x |] | destruct 1;
         apply remove_2]; auto.
        intro.
        apply (remove_1 H0 H).
      Qed.
      ...
      Proof.
        split; [apply union_1 | destruct 1;
         [apply union_2|apply union_3]]; auto.
      Qed.
      ...
      Proof.
        split; [split; [apply inter_1 with s' |
         apply inter_2 with s] | destruct 1; apply inter_3];
         auto.
      Qed.
    ...
    Hint Rewrite
      empty_iff singleton_iff add_iff remove_iff union_iff
      inter_iff diff_iff
    : set_simpl.
    ...
    Proof.
      intros x s. red.
       remember (M.mem x s) as b. symmetry in Heqb. destruct b.
        auto using M.mem_1.
        right; intros H.
         assert (M.mem x s = true) by auto using M.mem_2.
         rewrite -> Heqb in H0. discriminate H0.
    Qed.
    ...
    Lemma dec_eq : forall (x y : E.t),
      decidable (E.eq x y).
    Proof.
      intros x y. red. destruct (E.eq_dec x y); auto.
    Qed.
    ...
    (** We are going to write a tactic that works like the
        built-in tactic [subst], but at the level of set element
        equality (which may or may not be the convertible with
        Coq's equality). *)

    Section RewriteWithEq.
      Variables x y z : E.t.
      Variables s : t.

      Lemma rewr_eq_l :
        E.eq x y -> (E.eq x z <-> E.eq y z).
      Proof.
        intros eq; split; intros eq'.
          apply E.eq_sym in eq. apply (E.eq_trans eq eq').
          apply (E.eq_trans eq eq').
      Qed.

      Lemma rewr_eq_r :
        E.eq x y -> (E.eq z x <-> E.eq z y).
      Proof.
        intros eq; split; intros eq'.
          apply (E.eq_trans eq' eq).
          apply E.eq_sym in eq. apply (E.eq_trans eq' eq).
      ...
      Proof.
        intros eq; split; intros H.
          apply (In_1 eq H).
          apply E.eq_sym in eq. apply (In_1 eq H).
      Qed.
    ...
    Ltac substFSetElt x :=
      match goal with
      | eq1: E.eq x ?y |- _ =>
          if prop (E.eq x y) holds by apply E.eq_refl
          then fail
          else (
            repeat (progress (match goal with
            | |- context [E.eq x ?z] =>
                rewrite -> (rewr_eq_l x y z eq1)
            | |- context [E.eq ?z x] =>
                rewrite -> (rewr_eq_r x y z eq1)
            | |- context [In x ?s] =>
                rewrite -> (rewr_In x y s eq1)
            | eq2: context [E.eq x ?z] |- _ =>
                if prop (E.eq y z) holds by apply E.eq_refl
                then fail
                else rewrite -> (rewr_eq_l x y z eq1) in eq2
            | eq2: context [E.eq ?z x] |- _ =>
                rewrite -> (rewr_eq_r x y z eq1) in eq2
            | H: context [In x ?s] |- _ =>
                rewrite -> (rewr_In x y s eq1) in H
            end));
            match goal with
    ...
    Ltac fsetdec_body :=
      ...
      assert_decidability;
      substFSet;
      assert (forall x : E.t, E.eq x x) by exact E.eq_refl;
      auto;
      (intuition fsetdec_rec) ||
      fail 1
